...
m := NewMachine().Consult(`
  ...
  base_desc_ley(X,Y,P,rete384):- valor_pago(X,P,R), Y is (R rnd 0).
  ...
  valor_pago_neto(X,Y,P,V,L,L2):-valor_pago(X,P,V),valores(X,descuento,P,L),descuentos_ley(X,T,P,L2),unir(L,L2,LS),total_descuentos(LS,D),R is V - D, Y is (R rnd 0).
